<?php
	...
	$hint = "";
	$submittedCode = strtoupper($_REQUEST['w']);
	$BOOLListCode = false;
	$haveListBOOL = false;
	$increment = 0;
	$key1 = 0;
	...
	if(($BOOLListCode == true) && ($haveListBOOL == true))
	{
		
		mysqli_query($conn, "DELETE FROM havelist WHERE codes = '$submittedCode'");
		//mysqli_query($conn, "DELETE FROM havelist (codes, key1) VALUES ('$submittedCode', '$key1')");
		$hint = '<br> <span style = "color: red">' . $submittedCode . '</span> will be deleted automatically';
		//echo 'if true false';
